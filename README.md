...
- `MyRadio`, `MyRadioGroup`: Unity `Toggle`, `ToggleGroup`, `Selectable`에 기대지 않고 상태별 GameObject 표시와 명시 배열 기반 라디오 선택 UI를 구성하며, 라디오별 이미지와 텍스트는 serialized 배열 참조로 연결합니다. Inspector 미리보기와 플레이 모드 초기화 경계를 분리합니다.
