...
- `MyButton`: 클릭, pointer/focus 기반 hover 사운드, 호버, 프레스, 더블 클릭 콜백과 버튼 사운드 재생을 다루며 이미지 갱신 표면은 `MyImage` 기준으로 맞춥니다. 기본 disabled 이미지는 숨기고, 옵션을 켜면 이미지를 유지한 채 disabled 색상으로 표시합니다.
