...
- `MyReel`: 전체 값은 유지하면서 viewport 주변의 엔트리만 생성·재사용하는 가변 높이 목록입니다. 엔트리는 `Value` setter 안에서 `RectTransform.sizeDelta.y`를 동기적으로 계산해야 합니다.
...
- `MyText` / `MyImage` / `MyPortrait`: 기본 텍스트와 이미지 갱신을 감싸고, `MyText`의 preferred size 사전 계산과 선택적인 자동 너비·높이 조정, `MyImage.SetNativeSizeSprite`로 컨트롤 크기를 콘텐츠에 맞출 수 있습니다.
