...
패키지의 런타임 헬퍼 컴포넌트는 한 GameObject에 같은 타입을 하나만 두는 것을 기준으로 합니다. 화면에 직접 부착되는 컨트롤은 `RectTransform`을 요구하고, 같은 오브젝트에 필수 UGUI 컴포넌트가 있어야 하는 타입도 `RequireComponent`로 의존성을 표시합니다. EventSystem 감지·전달이나 임의 GameObject 상태만 조정하는 보조 컴포넌트는 `RectTransform`을 강제하지 않습니다. 헬퍼 컴포넌트 본체는 `DisallowMultipleComponent`로 중복 부착을 막습니다.
...
`HoverInterface`는 interactable 버튼에 pointer가 진입하면 `OnHoverEnter()`를 호출합니다. 진입 뒤 pointer가 나가거나 버튼이 non-interactable 또는 비활성 상태가 되면 `OnHoverExit()`을 한 번 호출하며, 이후 EventSystem의 늦은 이탈 이벤트가 도착해도 중복 호출하지 않습니다.
...
`MyBar`, `MySlider`, `MyToggle`, `MySelector`, `MySwapper`는 값 기반 UI를 갱신할 때 사용합니다. 각 컴포넌트는 필요한 초기화 또는 변경 콜백 인터페이스를 함께 제공합니다. `MySelector.HoverInterface`는 같은 GameObject에서 pointer 진입과 이탈을 받아 `OnHoverEnter()`와 `OnHoverExit()`로 전달하며, 진입 뒤 selector가 비활성화되면 이탈을 한 번 보충합니다.
...
대상 위쪽에 전체 높이가 들어가면 위에 배치하고, 그렇지 않으면 아래쪽을 사용합니다. 양쪽 모두 부족하면 더 넓은 쪽을 선택한 뒤 툴팁을 Canvas 안에 맞춥니다. 대상과 툴팁은 같은 root Canvas에 있어야 합니다.
